...
class generation(object):
    ...
    def move_firefly(self, x, y):
        ...
        r = abs(r**0.5)
        new_location = [(y.flies[i] + c.beta*(np.exp(-c.gamma*r))*(x.flies[i] - y.flies[i]) + c.alpha*np.random.normal(0.5, 0.3, 1)[0]) for i in range(0, c.no_of_var)]
        return new_location
        ...
        for x in self.population_members:
            ...
            for y in first_front:
                ...
                if b > brightest_value:
                    brightest_value = b
                    brightest_firefly = y
            child = self.move_firefly(brightest_firefly, x)
            new_fireflies += [firefly(child)]
        return new_fireflies

    def random_walk(self):
        # Initialising random weights
        print("Random weight Allocation")
        new_fireflies = []
        random_weights = np.random.dirichlet(np.ones(p.no_of_objectives), size=1)
        self.population_members[0].avg_brightness = [(random_weights[0][i] * self.population_members[0].brightness_values[i]) for i in range(0, p.no_of_objectives)]
        gbest = self.population_members[0].avg_brightness[0]
